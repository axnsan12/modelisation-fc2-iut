	...
	//Arbre de classification
		...
		//Afficher le tableau de contingence de deux vecteurs passes en parametres 
				...
				private static int[][] tabCont(int[] X, int[] Y) {
					int [][] tab;
					boolean opt01=false,opt02=false;
					int []totopt1=calculOption(X);
					int []totopt2=calculOption(Y);
					if (totopt1[0]==0){opt01=true;}
					if (totopt2[0]==0){opt02=true;}
					int som=0;
					...
					if (opt01)
					{
						if (opt02)
						{
							...
							{
								tab[Y[k]][X[k]] = tab[Y[k]][X[k]]+1;
								tab[Y[k]][totopt1.length] = tab[Y[k]][totopt1.length]+1;
								tab[totopt2.length][X[k]] = tab[totopt2.length][X[k]]+1;
